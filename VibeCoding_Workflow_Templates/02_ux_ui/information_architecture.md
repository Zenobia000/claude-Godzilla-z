...
> **相關文檔:** [PRD](../01_requirements/prd.md) | [前端架構](./frontend_technical_design.md)
>
> **MECE 邊界**：本文件**只談使用者/內容視角**（旅程、導航、頁面職責、URL、跨頁資料模型）。
> 技術視角（框架、效能數字、a11y 標準、檔案組織）全部在 **frontend_technical_design**。
>
> | 你想找的 | 看這份 |
> |---|---|
...
> 轉換率/事件追蹤在 frontend_technical_design §8（技術實作）；旅程本身（行為、情緒、痛點）在 ux_research_and_journey §4，本表只做旅程→頁面映射，本表只定義「追蹤什麼業務動作」。
