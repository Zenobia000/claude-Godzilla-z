...
這些問題**不代填**。它們的用途是逼出「你其實還沒決定的事」——答不出來的那一條，就是這份文件目前最大的洞。

**架構層**
- 若某兩個品質屬性衝突（例如延遲 vs 成本），誰優先？依據是什麼？
- 若單一依賴失效，系統如何退化仍滿足業務的最小價值？
- 哪些假設若被推翻，整份設計需要重構？（寫進「第 7 部分：風險與演進」的重評觸發）

**領域模型層（第 3 部分）**
- 這個聚合的**唯一交易邊界**是什麼？違反時會出現什麼不一致？
- 哪條規則是**不變量**（必須恆真）而非流程慣例（可以有例外）？如何破壞性驗證？
- 領域事件的命名是否貼合業務語彙，且描述「已發生」的事？

**資料層（第 4 部分）**
- 正規化到什麼程度？在什麼情境下會刻意反正規化？
- 索引是否過度影響寫入效能？有沒有更合適的索引類型？
- 大規模資料如何清除或封存？誰決定保留期限？

**實作邊界**
- 應用層與領域層是否完全不依賴基礎設施層的具體實作？
- 領域模型是否保持純淨，不含資料庫或框架相關程式碼？
- 錯誤處理是否能明確區分「業務規則錯誤」與「系統基礎設施錯誤」？
