...
L1→L2 的分水嶺同時是**需求決策的硬邊界**：優先序、範圍、里程碑、Gate、業務驗收由產品 owner 於 `VibeCoding_Workflow_Templates/01_requirements/requirement_decision_record.md`（追蹤視圖：`requirements_tracker.xlsx` ①需求決策）拍板，owner 未核准前 `/specify` 不得工程化。語域切換與決策交還是同一件事的兩面。
