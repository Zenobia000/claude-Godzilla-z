...
實作 code 與更新 docs 屬**同一個任務、同一個 PR**，不接受「以後再補文件」——之後再補幾乎都會忘，最後產生 doc drift。

POC 階段沒有強制的文件觸發表；只有一條線：**這次改動讓 `VibeCoding_Workflow_Templates/` 下哪份已填寫的文件失真，就一起改。** 沒填過的模板不需要為了這條規則去填。
