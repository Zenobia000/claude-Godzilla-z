---
name: sunnydata-wayfind
description: Chart a map for work too big and too foggy for one session — name the destination, ticket the decisions you can state now, and resolve them one per session until the way is clear enough to plan and build. Use when a greenfield project or large feature cannot be held in one session and the questions themselves are not yet clear.
...
`sunnydata-design` Phase 1 探索的是一個你**握得住**的問題。這個 Skill 處理的是你握不住的那種——大到一個 session 裝不下，而且**連要問什麼都還不確定**。它的產出是**決策，不是交付物**，也不是方案。
...
1. **命名終點。** 用一問一答把「這張圖要找到什麼」釘死——一份可以開始規劃的需求輪廓、一個要先鎖定的技術決策、或一次就地的改動。**終點決定範圍，所以先定終點。**
2. **掃前緣。** 再問一輪，這次**廣度優先**：橫向掃過整個空間，而不是往任何一條線深挖，找出開放的決策與現在就能踏出的第一步。
   **如果掃不出霧**——路已經清楚、整趟小到一個 session 就能走完——**你不需要地圖**。停下來告訴使用者，建議直接走 `sunnydata-design`。
...
3. 解它——**按需放大**：要用到哪張相關或已關閉的 ticket，才去讀它的完整內容；載入 `Notes` 指名的 skill。
...
| 終點是 | 交給 |
|---|---|
| 一組需求輪廓，還要探索方案 | `sunnydata-design` Phase 1（此時問題已經握得住了）|
| 方案已定，只差拆解與實作計畫 | `sunnydata-design` Phase 2（切片與任務）|
| 一次就地的改動（資料結構遷移之類） | 直接做，但先開分支 |

**不要把地圖直接迴圈進實作。** 地圖的價值在那些連結的決策細節；跳過收束等於把它們丟掉。只有在 effort 最後證明其實很小的時候，才直接動手。
...
移植自 [mattpocock/skills](https://github.com/mattpocock/skills) `wayfinder`（MIT, © 2026 Matt Pocock）。地圖改為 repo 內的 markdown＋單一 ticket 表（上游用 issue tracker 的原生 blocking 與 query）、L1/L2 語域約束、與 `sunnydata-design` 兩階段的交棒對照為本專案新增。
